...
var textoOriginal = prompt("Digite um texto qualquer: ", "Eu amo abacaxi (amora, mexerica). Eu gosto de moranga. Você gosta de batata? Gosto também de cebola na comida. Cebola (alho) é bom demais.");
        ...
        for (var i = 0; i < texto.length; i++) {
            // Converte a palavra atual para letras minúsculas para comparar
            var palavra = texto[i].toUpperCase();

            // Verifica se a palavra está na lista de frutas
            if (vetFrutas.includes(palavra)) {
                frutas.push(texto[i]);
            }

            // Verifica se a palavra está na lista de verduras
            if (vetLegumes.includes(palavra)) {
                verduras.push(texto[i]);
            }
        }

        document.write(`<h2>Frutas encontradas: ${frutas.length} </h2>`)
        document.write(`<p>${frutas.join(", ")}</p>`)

        document.write(`<h2>Verduras encontradas: ${verduras.length}</h2>`)
        document.write(`<p>${verduras.join(", ")}</p>`)

        document.write(`<h3>Estatísticas:</h3>`)

        document.write(`<p><strong>Texto original:</strong> ${textoOriginal}</p>`)
        document.write(`<p><strong>Total (palavras analisadas):</strong> ${texto.length}</p>`)

        var frases = textoOriginal.match(/[^.!?]+[.!?]+/g);

        document.write(`<h3>Frases encontradas:</h3>`)
        document.write(`<ul>`);
        for (var i = 0; i < frases.length; i++) {
            document.write(`<li>${frases[i]}</li>`);
        }
        document.write(`</ul>`);
        document.write(`<p><strong>Total de frases:</strong> ${frases.length}</p>`)
